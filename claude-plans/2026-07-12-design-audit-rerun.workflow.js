// Stage-4 RE-RUN variant (2026-07-12) of the banked design-audit workflow.
// Two deltas from the original, both from the discovery run's named coverage
// gaps: (1) walkers first open args.authUrl — the Vercel preview is
// deployment-protected and the _vercel_share hit sets the auth cookie;
// (2) a dedicated OFFLINE prober (chrome-devtools MCP) covers F15 for real,
// and F13 is walked end-to-end against the seeded Client ID.
export const meta = {
  name: 'design-audit-rerun',
  description: 'Stage-4 re-run: walk every flow at 3 viewports + offline prober, review, adversarially verify, synthesize',
  phases: [
    { title: 'Recon', detail: '3 viewport walkers + offline prober + 4 code finders' },
...
AUTH FIRST: the deployment is protected — navigate to ${AUTH_URL} once before anything else; it sets an auth cookie and redirects into the app. Every later navigation to ${BASE} then works. If you ever see a Vercel login page, re-open ${AUTH_URL}.
...
- F13: open the Spotify drill — the crew HAS a Client ID seeded, so you should see the ready/connect state. Screenshot every state you can reach, including the crew-app config row. Click Connect and screenshot whatever page results (do NOT log in to Spotify — an error page from Spotify is itself evidence the flow reached OAuth); browser-back to the app.
- F14: open export/download tools, screenshot; run the Day image export for one day, note what happens.
- F15: skip in this walker; a dedicated offline prober covers it.
...
const OFFLINE_PROBER = `${COMMON}
You are the OFFLINE prober (spec F15 — the discovery run's named coverage gap). Use the chrome-devtools MCP browser (load tools via ToolSearch, e.g. "select:mcp__plugin_chrome-devtools-mcp_chrome-devtools__new_page,mcp__plugin_chrome-devtools-mcp_chrome-devtools__navigate_page,mcp__plugin_chrome-devtools-mcp_chrome-devtools__emulate,mcp__plugin_chrome-devtools-mcp_chrome-devtools__take_screenshot,mcp__plugin_chrome-devtools-mcp_chrome-devtools__evaluate_script" plus click/snapshot/wait_for as needed).
1. Open ${AUTH_URL} (sets the deployment auth cookie), then ${LINK}. If a join screen appears, claim the EXISTING member "Auditor" (tap their row).
2. Wait for the wall; give the service worker ~3s to install (evaluate: navigator.serviceWorker.ready), then reload once so the SW controls the page. Screenshot to ${SHOTS}/offline/01-online-wall.png (verify the path with Bash ls after the first shot; adjust if the MCP saved elsewhere).
3. Emulate OFFLINE via the emulate tool (network: offline / 'No emulation'->offline preset).
4. Reload. Screenshot 02-offline-reload.png. Report honestly: did the app shell render (wall visible) or an error page? What does the sync dot / any status show?
5. Still offline: tap one artist card. Screenshot 03-offline-pick.png. Did the pick apply visually?
6. Still offline: open Settings and switch to a festival that was NOT opened this session (e.g. EDC ORLANDO). Report exactly what happens — the spec says the app must not strand (expected: an explanatory toast, no switch).
7. Go back online (emulate: no throttling). Wait ~30s (or dispatch window online event via evaluate), then verify the step-5 pick reached the server: evaluate fetch('/api/crew?t=' + the token from ${LINK}).then(r=>r.json()) and check the selections for the artist you tapped under "Auditor". Report found/not-found.
8. Screenshot 04-back-online.png. Append a step-by-step log to ${SHOTS}/offline/walk-log.md as you go.
Return findings via schema: each broken/confusing/dishonest state is a finding with evidence (shot filename or observed value); an empty findings list means offline held up.`
...
const reconResults = await parallel([
  ...
  () => agent(OFFLINE_PROBER, { label: 'walk:offline', phase: 'Recon', schema: FINDINGS_SCHEMA, model: 'sonnet' }),
...
const offlineFindings = (reconResults[3] && reconResults[3].findings) || []
const codeFindings = reconResults.slice(4).filter(Boolean).flatMap((r) => r.findings).concat(offlineFindings)
log(`Recon done: ${walks.length}/3 walks, ${offlineFindings.length} offline findings, ${codeFindings.length} code+offline findings`)
