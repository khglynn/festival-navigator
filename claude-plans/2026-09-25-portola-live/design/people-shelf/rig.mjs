// The people-menu frame rig (2026-09-25, on v92; the OURS rig, plus a guest,
// a seeded highlight and no welcome card): boots the REAL app in one headless
...
const TOKEN = 'peoplemenuframes_01234567'; // made up; never a real crew link
...
export async function openApp(rig, { now, width = 390, height = 844, fold = null, crew = 9, desktop = false, guest = false, highlight = null } = {}) {
  ...
  await ctx.addInitScript(([t, me, folded, hl]) => {
    ...
    if (me) localStorage.setItem(`fn_me_v3_${t}`, me);
    localStorage.setItem('fn_welcome_v1', '1');
    localStorage.setItem('fn_welcome_joined_v1', '1');
    if (hl) sessionStorage.setItem('fn_filter_people_v1_portola-2026', JSON.stringify(hl));
  ...
  }, [TOKEN, guest ? null : ME, fold, highlight]);
