// The OURS frame rig (2026-09-25): boots the REAL app in one headless
...
const TOKEN = 'oursdesignframes_0123456789'; // made up; never a real crew link
...
export async function openApp(rig, { now, width = 390, height = 844, fold = null } = {}) {
  const ctx = await rig.browser.newContext({ viewport: { width, height }, deviceScaleFactor: 2, hasTouch: true, isMobile: true, serviceWorkers: 'block' });
  ...
  await ctx.route(`${origin}/api/crew**`, (route) => (route.request().method() === 'GET'
    ? route.fulfill({ contentType: 'application/json', body: JSON.stringify(crewDoc()) })
